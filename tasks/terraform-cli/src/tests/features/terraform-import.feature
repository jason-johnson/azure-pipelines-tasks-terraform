...
Feature: terraform import
    ...
    Scenario: import with azurerm and command options with escaped quotes
        Given terraform exists
        And terraform command is "import" with options "-var=\"resource_group_location=location\" -var=\"resource_group_name=name\""
        And azurerm service connection "dev" exists as
            | scheme         | ServicePrincipal       |
            | subscriptionId | sub1                   |
            | tenantId       | ten1                   |

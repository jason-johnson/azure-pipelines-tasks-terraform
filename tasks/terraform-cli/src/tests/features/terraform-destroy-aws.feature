Feature: terraform destroy for aws

  terraform destroy [options] [dir]

  Scenario: destroy without aws service connection
    Given terraform exists
    And terraform command is "destroy"
    And running command "terraform destroy -auto-approve" returns successful result
    When the terraform cli task is run
    Then the terraform cli task executed command "terraform destroy -auto-approve" without the following environment variables
      | AWS_ACCESS_KEY_ID     |
